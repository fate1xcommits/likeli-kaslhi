...
class DFlowAPI {
    ...
    // Mock data for development with LIVE AMM prices
    getMockData(endpoint) {
        // Import AMM functions inline (since this is a class method)
        const getAmmPrice = (marketId) => {
            try {
                const stored = localStorage.getItem('likeli_amm_markets')
        ...
        const getMarketVolume = (marketId) => {
            try {
                const stored = localStorage.getItem('likeli_amm_markets')
                if (!stored) return 0
                const pools = JSON.parse(stored)
                return pools[marketId]?.totalVolume || 0
            } catch {
                return 0
            }
        }
        ...
        const mockEvents = [
            {
                ...
                volume: 1874290 + getMarketVolume('NE-BAL'),
                markets: [
                    {
                        id: 'NE-BAL', title: 'Winner', outcomes: (() => {
                            const yesPrice = getAmmPrice('NE-BAL') ?? 0.38
                            return [
                                { name: 'NE', abbr: 'NE', price: yesPrice, change: 0 },
                                { name: 'BAL', abbr: 'BAL', price: 1 - yesPrice, change: 0 }
                            ]
                        })()
                    }
                ]
            },
            {
                ...
                volume: 1313116 + getMarketVolume('BUF-CLE'),
                markets: [
                    {
                        id: 'BUF-CLE', title: 'Winner', outcomes: (() => {
                            const yesPrice = getAmmPrice('BUF-CLE') ?? 0.84
                            return [
                                { name: 'BUF', abbr: 'BUF', price: yesPrice, change: 0.02 },
                                { name: 'CLE', abbr: 'CLE', price: 1 - yesPrice, change: -0.02 }
                            ]
                        })()
                    }
                ]
            },
            {
                ...
                status: 'open',
                endDate: '2024-12-22T13:00:00Z',
                volume: 709275 + getMarketVolume('PIT-DET'),
                markets: [
                    {
                        id: 'PIT-DET', title: 'Winner', outcomes: (() => {
                            const yesPrice = getAmmPrice('PIT-DET') ?? 0.26
                            return [
                                { name: 'PIT', abbr: 'PIT', price: yesPrice, change: 0 },
                                { name: 'DET', abbr: 'DET', price: 1 - yesPrice, change: 0 }
                            ]
                        })()
                    }
                ]
            },
            {
                ...
                status: 'open',
                endDate: '2024-12-22T13:00:00Z',
                volume: 635790 + getMarketVolume('MIN-NYG'),
                markets: [
                    {
                        id: 'MIN-NYG', title: 'Winner', outcomes: (() => {
                            const yesPrice = getAmmPrice('MIN-NYG') ?? 0.59
                            return [
                                { name: 'MIN', abbr: 'MIN', price: yesPrice, change: 0 },
                                { name: 'NYG', abbr: 'NYG', price: 1 - yesPrice, change: 0 }
                            ]
                        })()
                    }
                ]
